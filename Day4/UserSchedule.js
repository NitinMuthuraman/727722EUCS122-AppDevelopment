...
import { Box, Modal, Backdrop, Fade, Button, Typography, TextField } from '@mui/material';
import axios from 'axios';
import '../assets/Calendar.css';
import EventComponent from './EventComponent'; // Import the custom event component
...
const CalendarPage1 = () => {
  ...
  const handleSubmitRequest = async () => {
    try {
      ...
      setReason('');
      navigate('/user/requests'); // Navigate to the requests page
    } catch (error) {
      ...
      {selectedEvent && (
        ...
        >
          <Fade in={showEventDetailsModal}>
            <Box className="cal-modal-content" sx={{ width: '500px' }}> {/* Increase the width of the modal */}
              ...
              <Box mt={2} display="flex" justifyContent="space-between" gap={2}> {/* Add gap between buttons */}
