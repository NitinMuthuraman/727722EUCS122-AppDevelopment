...
import '../assets/Request.css'; // Ensure this file contains the required styles
  ...
  return (
    <Box>
      <Box display="flex" alignItems="center" justifyContent="space-between" padding="16px" boxShadow="0px 1px 2px rgba(0, 0, 0, 0.1)">
        ...
        <div style={{ width: 48 }} /> {/* To balance the header */}
      </Box>
      <Box padding="16px" display="flex" flexDirection="row" flexWrap="wrap" gap="16px">
        {requests.length === 0 ? (
          ...
          requests.map((request) => {
            const approvalStatus = request.approval || 'Pending'; // Default to 'Pending' if approval is undefined
            return (
              <Card
                key={request.id}
                variant="outlined"
              ...
              >
                <CardContent>
                  ...
                  {/* <Typography variant="body2">Day: {request.day}</Typography> */}
